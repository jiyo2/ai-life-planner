...
module.exports = async (req, res) => {
    ...
    const prompt = `You are an expert travel planner. Create an optimized trip strategy for ${days} days in ${destination} with a maximum spending cap of $${budget} for ${travelers} traveler(s). 
    ...
    1. The official Star Rating (e.g., ⭐⭐⭐⭐ 4-Star Hotel).
    ...
    Provide the final response strictly in JSON format with raw HTML markup natively built inside the string properties so it outputs perfectly in the layout. Return EXACTLY this structured JSON format:
    {
      "stay": "HTML markup containing styled list items of hotels with star ratings, room amenities (Internet, Bathroom, etc.), and their custom Booking.com links.",
      "transport": "HTML markup summarizing public transit cards, taxis, and structural routing costs.",
      "experiences": "HTML markup with curated daily lists of attractions, reservation tips, and local food venues.",
    ...
    try {
        ...
        if (!response.data || !response.data.candidates) {
            throw new Error("Invalid response structural block from Gemini API.");
        }

        let rawText = response.data.candidates.content.parts.text;
        ...
        return res.status(500).json({ 
            error: "Failed to assemble travel parameters securely.",
